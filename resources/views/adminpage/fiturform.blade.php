{{-- resources/views/adminpage/fiturform.blade.php --}}

<x-adminpage.mainlayout>
    <x-slot:title>
        {{-- Dynamic Title --}}
        {{ isset($fitur) ? 'Edit Fitur' : 'Tambah Fitur Baru' }}
    </x-slot:title>

    {{-- <x-slot:header>
        <img src="https://dummyimage.com/1200x300/3498db/ffffff&text=Form+Manajemen+Fitur" alt="Form Header">
    </x-slot:header> --}}
    ...
    <x-slot:content>
        <div class="bg-white p-8 rounded-lg shadow-md ">
            ...
            {{-- Dynamic Form Action --}}
            <form action="{{ isset($fitur) ? route('fitur.update', $fitur->id) : route('fitur.store') }}" method="POST"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="title" id="title"
                            class="mt-1 block w-full rounded-mdshadow-sm @error('title') border-red-500 @else border-gray-300 @enderror"
                            value="{{ old('title', $fitur->title ?? '') }}">
                        @error('title')
                    ...
                    <div>
                        ...
                        <select name="type" id="type"
                            class="mt-1 block w-full rounded-md shadow-sm @error('type') border-red-500 @else border-gray-300 @enderror">

                            @php
                                // Set a default value for new forms, or use the existing one
                                $selectedType = old('type', $fitur->type ?? 'boolean');
                            @endphp

                            <option value="boolean" @if ($selectedType == 'boolean') selected @endif>Boolean (On/Off)
                            </option>
                            <option value="string" @if ($selectedType == 'string') selected @endif>String (Teks)
                            </option>
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Ikon File</label>
                        <div class="flex items-center justify-center w-full">
                            <label for="icon" id="dropzone-label"
                                class="flex flex-col items-center justify-center w-full h-48 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 @error('icon') border-red-500 @enderror">
                                ...
                                {{-- IMPORTANT: The `name` is "icon" for the controller, and `id` matches the label's `for` --}}
                        ...
                        @if (isset($fitur) && $fitur->icon)
                            <div class="mt-4">
                                <p class="text-sm font-medium text-gray-500">Ikon Saat Ini:</p>
                                @if (Str::endsWith($fitur->icon, '.pdf'))
                                    <a href="{{ asset('storage/' . $fitur->icon) }}" target="_blank"
                                        class="text-blue-600 hover:underline">Lihat PDF</a>
                                @else
                                    <img src="{{ asset('storage/' . $fitur->icon) }}" alt="Current Icon"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input id="description" type="hidden" name="description"
                            value="{{ old('description', $fitur->description ?? '') }}">
                        <trix-editor input="description"
                            class="mt-1 block w-full rounded-md shadow-sm bg-white @error('description') border-red-500 @else border-gray-300 @enderror"></trix-editor>
                    ...
                    <div>
                        <label for="is_active" class="flex items-center">
                            <input type="checkbox" name="is_active" id="is_active" value="1"
                                ...
                                @if (old('is_active', $fitur->is_active ?? true)) checked @endif>
