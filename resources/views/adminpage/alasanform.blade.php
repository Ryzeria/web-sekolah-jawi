{{-- resources/views/adminpage/alasanform.blade.php --}}

<x-adminpage.mainlayout>
    <x-slot:title>
        {{-- Dynamic Title --}}
        {{ isset($alasan) ? 'Edit Alasan' : 'Tambah Alasan Baru' }}
    </x-slot:title>

    {{-- <x-slot:header>
        <img src="https://dummyimage.com/1200x300/3498db/ffffff&text=Form+Manajemen+Alasan" alt="Form Header">
    </x-slot:header> --}}
    ...
    <x-slot:content>
        <div class="bg-white p-8 rounded-lg shadow-md">
            ...
            {{-- Dynamic Form Action --}}
            <form action="{{ isset($alasan) ? route('alasan.update', $alasan->id) : route('alasan.store') }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="urutan" id="urutan"
                            ...
                            value="{{ old('urutan', $alasan->urutan ?? '') }}">
                        @error('urutan')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="md:col-span-2">
                        ...
                        @if (isset($alasan) && $alasan->icon)
                            <div class="mt-4">
                                <p class="text-sm font-medium text-gray-500">Ikon Saat Ini:</p>
                                @if (Str::endsWith($alasan->icon, '.pdf'))
                                    <a href="{{ asset('storage/' . $alasan->icon) }}" target="_blank"
                                        class="text-blue-600 hover:underline">
                                        Lihat PDF: {{ basename($alasan->icon) }}
                                    </a>
                                @else
                                    <img src="{{ asset('storage/' . $alasan->icon) }}" alt="Current Icon"
                                        class="h-16 w-auto rounded-md mt-2 object-cover">
                                @endif
                            </div>
                        @endif
                    </div>
                    <div class="md:col-span-2">
                        ...
                        <input id="description" type="hidden" name="description"
                            value="{{ old('description', $alasan->description ?? '') }}">
